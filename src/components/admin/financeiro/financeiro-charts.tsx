...
function ChartCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-ds-lg border border-ds-border bg-ds-surface p-4">
      <h3 className="mb-4 font-ds-sans text-sm font-semibold text-ds-text">{title}</h3>
        ...
        {!serieTemDado ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={serie} margin={{ top: 4, right: 8, left: -8, bottom: 0 }}>
              ...
              <XAxis dataKey="mes" tick={{ fill: chart.axis, fontSize: 12, fontFamily: "var(--font-geist-mono)" }} tickLine={false} axisLine={false} />
              <YAxis tickFormatter={brlCompact} tick={{ fill: chart.axis, fontSize: 11, fontFamily: "var(--font-geist-mono)" }} tickLine={false} axisLine={false} width={56} />
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => brlFull(v)} cursor={{ fill: "rgba(255,255,255,0.04)" }} />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Bar dataKey="investimento" name="Investimento" fill={chart.secondary} radius={[3, 3, 0, 0]} />
              <Bar dataKey="receita" name="Receita" fill={chart.accent} radius={[3, 3, 0, 0]} />
              <Bar dataKey="lucro" name="Lucro" fill={chart.money} radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </ChartCard>
        ...
        {lucroPorTipo.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={lucroPorTipo} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 0 }}>
              ...
              <XAxis type="number" tickFormatter={brlCompact} tick={{ fill: chart.axis, fontSize: 11, fontFamily: "var(--font-geist-mono)" }} tickLine={false} axisLine={false} />
              <YAxis type="category" dataKey="tipo" tick={{ fill: chart.axis, fontSize: 12, fontFamily: "var(--font-geist-mono)" }} tickLine={false} axisLine={false} width={90} />
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => brlFull(v)} cursor={{ fill: "rgba(255,255,255,0.04)" }} />
              <Bar dataKey="lucro" name="Lucro" fill={chart.money} radius={[0, 3, 3, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </ChartCard>
